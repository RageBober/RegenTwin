...
export default function CellHeatmap({ simulationId }: Props) {
  ...
  return (
    <div data-testid="cell-heatmap">
      <div className="mb-3 flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          <label className="text-xs text-slate-500 dark:text-slate-400">{t('viz.binSize')}</label>
          <input
            type="range"
            min={5}
            max={25}
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            onChange={(e) => setSelectedTypes(e.target.value === 'all' ? null : [e.target.value])}
