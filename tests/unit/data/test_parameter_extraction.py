...
class TestParameterExtractorExtractC0:
    ...
    def test_extract_c0_increases_with_stem_cells(self):
        ...
        gates_low = {
            "cd34_positive": GateResult(
                name="cd34_positive", mask=np.array([True] * 100), n_events=100, fraction=0.01
            ),
            "macrophages": GateResult(
                name="macrophages", mask=np.array([True] * 300), n_events=300, fraction=0.03
            ),
            "live_cells": GateResult(
                name="live_cells", mask=np.array([True] * 7000), n_events=7000, fraction=0.70
            ),
        }
        ...
        gates_high = {
            "cd34_positive": GateResult(
                name="cd34_positive", mask=np.array([True] * 1000), n_events=1000, fraction=0.10
            ),
            "macrophages": GateResult(
                name="macrophages", mask=np.array([True] * 300), n_events=300, fraction=0.03
            ),
    ...
    def test_extract_c0_formula_correctness(self):
        ...
        config = ExtractionConfig(
            ref_cytokine_conc=10.0, stem_cell_cytokine_factor=2.0, macrophage_cytokine_factor=1.5
        )
        extractor = ParameterExtractor(config=config)

        gates = {
            "cd34_positive": GateResult(
                name="cd34_positive", mask=np.array([True] * 500), n_events=500, fraction=0.05
            ),
            "macrophages": GateResult(
                name="macrophages", mask=np.array([True] * 300), n_events=300, fraction=0.03
            ),
...
class TestExtractModelParametersFunction:
    ...
    def test_extract_model_parameters_returns_model_parameters(self, mock_gating_results_normal):
        """Тест что возвращается ModelParameters."""
    ...
    def test_extract_model_parameters_with_source_file(self, mock_gating_results_normal):
        ...
        params = extract_model_parameters(mock_gating_results_normal, source_file="sample.fcs")
...
class TestParameterExtractionScenarios:
    ...
    def test_parameters_consistency_across_scenarios(
        ...
        assert (
            params_regen.inflammation_level
            < params_normal.inflammation_level
            < params_inflamed.inflammation_level
        )

    def test_all_parameters_in_valid_ranges(
        self, mock_gating_results_normal, expected_parameter_ranges
    ):
        ...
        assert (
            ranges["stem_cell_fraction"]["min"]
            <= params.stem_cell_fraction
            <= ranges["stem_cell_fraction"]["max"]
        )
        assert (
            ranges["macrophage_fraction"]["min"]
            <= params.macrophage_fraction
            <= ranges["macrophage_fraction"]["max"]
        )
        assert (
            ranges["apoptotic_fraction"]["min"]
            <= params.apoptotic_fraction
            <= ranges["apoptotic_fraction"]["max"]
        )
...
class TestFromBasicParameters:
    ...
    def test_high_inflammation_high_C_TNF(self):
        ...
        config = ExtractionConfig()
        extended = ExtendedModelParameters.from_basic_parameters(basic)
        assert config.ref_TNF < extended.C_TNF
...
class TestExtractExtended:
    ...
    def test_8_gate_results_returns_extended(self, mock_gating_results_extended_normal):
...
class TestExtractNeutrophilFraction:
    ...
    def test_has_neutrophils_returns_float(self, mock_gating_results_extended_normal):
        """Тест что наличие neutrophils → float."""
        extractor = ParameterExtractor()
        result = extractor.extract_neutrophil_fraction(mock_gating_results_extended_normal)
        assert isinstance(result, float)

    def test_missing_neutrophils_raises_key_error(self, mock_gating_results_normal):
...
class TestExtractEndothelialFraction:
    ...
    def test_has_endothelial_returns_float(self, mock_gating_results_extended_normal):
        """Тест что наличие endothelial → float."""
        extractor = ParameterExtractor()
        result = extractor.extract_endothelial_fraction(mock_gating_results_extended_normal)
        assert isinstance(result, float)

    def test_missing_endothelial_raises_key_error(self, mock_gating_results_normal):
...
class TestEstimateCytokineProfile:
    ...
    def test_high_macro_high_TNF(self, mock_gating_results_inflamed):
        ...
        config = ExtractionConfig()
        extractor = ParameterExtractor()
        result = extractor.estimate_cytokine_profile(mock_gating_results_inflamed)
